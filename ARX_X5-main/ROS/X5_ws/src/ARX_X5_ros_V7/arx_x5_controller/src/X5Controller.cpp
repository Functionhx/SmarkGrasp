...
X5Controller::X5Controller(ros::NodeHandle nh) {
  ...
  std::string arm_control_type =
      nh.param("arm_control_type", std::string("normal_v1"));
  ...
  can_name_ = nh.param("arm_can_id", std::string("can1"));
  interfaces_ptr_ =
...
void X5Controller::PubState(const ros::TimerEvent &) {
  ...
  // 发布消息
  ROS_INFO("--------------------------------\n\n");
  ROS_INFO("joint_pos: %.6f, %.6f, %.6f, %.6f, %.6f, %.6f, %.6f",
           joint_pos_vector[0], joint_pos_vector[1], joint_pos_vector[2],
           joint_pos_vector[3], joint_pos_vector[4], joint_pos_vector[5],
           joint_pos_vector[6]);
  ROS_INFO("joint_vel: %.6f, %.6f, %.6f, %.6f, %.6f, %.6f, %.6f",
           joint_velocities_vector[0],
