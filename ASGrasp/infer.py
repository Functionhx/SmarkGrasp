...
class RealTimeGraspDetector:
    ...
    def try_infer(self):
        ...
        def infer_thread():
            try:
                print("收到全部图像，开始推理...")
                self.eval.infer_cv(rgb, ir1, ir2, mask)
...
def infer_cv(self, rgb_img, ir1_img, ir2_img, mask_img, zrot=None):
    ...
    with torch.no_grad():
        ...
        if gg is not None and len(gg) > 0:
            ...
            print("\n===== 抓取姿态信息(camera) =====")
            print(f"夹爪位置 (x, y, z): ({pos_B[0]:.4f}, {position[1]:.4f}, {position[2]:.4f}) 米")
            print(f"夹爪姿态 (弧度): roll={math.radians(roll):.4f}, pitch={math.radians(pitch):.4f}, yaw={math.radians(yaw):.4f}")
            print("夹爪姿态 (旋转矩阵):")
        ...
        print(f"本次推理耗时: {end_time - start_time:.3f} 秒")
        return gg
