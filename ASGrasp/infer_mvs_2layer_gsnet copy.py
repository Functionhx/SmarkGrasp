...
class MVSGSNetEval():
    ...
    def infer(self, rgb_path, ir1_path, ir2_path, mask_path, zrot=None):
        with torch.no_grad():
            ...
            idxs = np.random.choice(len(cloud_masked), 10000, replace=False)
            ...
            depth_mask1 = (depth1 > 0.20) & (depth1 < 5) & (depth1-depth>0.001)
            ...
            if (len(cloud_masked1) > 0):
                ...
                objectness_label = np.concatenate([np.ones([10000, ]), (-1)*np.ones([comp_num_pt, ])], axis=0)
                cloud_masked = np.concatenate([cloud_masked, cloud_masked1], axis=0)
            else:
                objectness_label = np.ones([10000, ])
            ...
            if gg is not None and len(gg) > 0:
                ...
                print("\n===== 抓取姿态信息 =====")
                print(f"夹爪位置 (x, y, z): ({position[0]:.4f}, {position[1]:.4f}, {position[2]:.4f}) 米")
                print("夹爪姿态 (旋转矩阵):")
                ...
                print(f"夹爪张开宽度: {grasp.width:.4f} 米")
                print(f"抓取置信度: {grasp.score:.4f}")
            else:
                print("未检测到有效抓取")           
...
if __name__ == '__main__':
    ...
    parser.add_argument('--num_point', type=int, default=10000, help='Point Number [default: 15000]')
    ...
    eval = MVSGSNetEval(args)
      ...
      # test inputs
    rgb_path = f'./test_data/rgb_20250725_142714.png'
    ir1_path = f'./test_data/left_ir_20250725_142714.png'
    ir2_path = f'./test_data/right_ir_20250725_142714.png' 
    mask_path = f'./test_data/bottle.png'
    
    # Run inference and visualize top-1 grasp
    top_grasp = eval.infer(rgb_path, ir1_path, ir2_path,mask_path)
