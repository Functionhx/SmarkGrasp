...
double GetPathsCost (const nav_msgs::Path& new_path, const nav_msgs::Path& old_path, 
                      int index, int time = 20){
  
  double cost = 0.0;
  for (int i = 0; i < time; i++){
    if (index + i < old_path.poses.size() && i < new_path.poses.size())
      cost += GetEuclideanDistance(new_path.poses[i], old_path.poses[index + i]);
    else{
      time = i;
      break;
    }
